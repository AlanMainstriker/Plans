...
def edit_task(request: Request, user_id: int, project_id: int, task_id: int, newdesc=None):
    global CURRENT_USER
    if user_id == CURRENT_USER:
        session = Session(bind=engine)
...
def edit_project(request: Request, user_id: int, project_id: int, newname=None):
    global CURRENT_USER
    if user_id == CURRENT_USER:
        session = Session(bind=engine)
        project_request = select(Project).where(Project.id == project_id)
...
def delete_task(request: Request, user_id: int, project_id: int, task_id: int):
    global CURRENT_USER
...
def delete_project(request: Request, user_id: int, project_id: int):
    global CURRENT_USER
    if user_id == CURRENT_USER:
        session = Session(bind=engine)
        project_request = select(Project).where(Project.id == project_id)
...
def task_done(request: Request, user_id: int, project_id: int, task_id: int):
    global CURRENT_USER
    if user_id == CURRENT_USER:
        session = Session(bind=engine)
        task_request = select(Task).where(Task.id == task_id)
...
def hide_projects(request: Request, user_id: int):
    global CURRENT_USER
    if user_id == CURRENT_USER:
        session = Session(bind=engine)
        user_request = select(Users).where(Users.id == user_id)
...
def hide_tasks(request: Request, user_id: int, project_id: int):
    global CURRENT_USER
    if user_id == CURRENT_USER:
        session = Session(bind=engine)
